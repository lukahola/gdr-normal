...
class GDRN(nn.Module):
    ...
    def forward(
        ...
        if self.concat:
            ...
            mask, norm_stc_x, norm_stc_y, norm_stc_z, norm_dyn_x, norm_dyn_y, norm_dyn_z, region = self.rot_head_net(features, x_f64, x_f32, x_f16)
        else:
            features = self.backbone(x)  # features.shape [bs, 2048, 8, 8]
            # joints.shape [bs, 1152, 64, 64]
            mask, norm_stc_x, norm_stc_y, norm_stc_z, norm_dyn_x, norm_dyn_y, norm_dyn_z, region = self.rot_head_net(features)
        ...
        if not do_loss:  # test
            ...
            if cfg.TEST.USE_SVD:
                ...
                out_dict.update({"mask": mask, "norm_stc_x": norm_stc_x, "norm_stc_y": norm_stc_y, "norm_stc_z": norm_stc_z, "norm_dyn_x": norm_dyn_x, "norm_dyn_y": norm_dyn_y, "norm_dyn_z": norm_dyn_z, "region": region, "rot_allo": pred_rot_m})
        else:
            out_dict = {}
            assert (
                (gt_norm is not None)

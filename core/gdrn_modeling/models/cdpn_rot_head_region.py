        ...
        if self.concat:
            ...
            if self.freeze:
                with torch.no_grad():
                    ...
                    norm_dyn_y = norm[:, 4, :, :, :]
                    norm_dyn_z = norm[:, 5, :, :, :]
                    return (mask.detach(), norm_stc_x.detach(), norm_stc_y.detach(), norm_stc_z.detach(), norm_dyn_x.detach(), norm_dyn_y.detach(), norm_dyn_z.detach(), region.detach())
            else:
                for i, l in enumerate(self.features):
                    x = l(x)
                mask = x[:, : self.mask_output_dim, :, :]
                ...
                norm_dyn_y = norm[:, 4, :, :, :]
                norm_dyn_z = norm[:, 5, :, :, :]
                return mask, norm_stc_x, norm_stc_y, norm_stc_z, norm_dyn_x, norm_dyn_y, norm_dyn_z, region
